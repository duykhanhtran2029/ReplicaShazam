	...
	public partial class Shazam
	{
		...
		private double[] RecordAudio(int length)
		{
			...
			using (var recorder = new SoundBufferRecorder())
			{
				...
				Task waiter = new Task(() => Thread.Sleep(length)); //record for length millisecs
				waiter.Start();
				...
				recorder.Stop();
				Console.WriteLine("{0,2} {1}", "", "Fingerprinting audio...");
		...
		/// <returns></returns>
		private List<TimeFrequencyPoint> CreateTimeFrequencyPoints(int bufferSize, double[] data, double sensitivity = 0.9)
		{
		...
		private void RegisterTFPoints(double[] data, in double average, in uint absTime, ref List<TimeFrequencyPoint> timeFrequencyPoitns, double coefficient = 0.9)
		{
			int[] BinBoundries =
			{
				//low   high
